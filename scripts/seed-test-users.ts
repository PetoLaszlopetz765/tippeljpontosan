...
async function main() {
  ...
  const admin = await prisma.user.create({ data: { username: "admin", password: "ToleDo1975", role: "ADMIN" } });
  const users = await Promise.all([
    prisma.user.create({ data: { username: "user1", password: "pw", role: "USER" } }),
    prisma.user.create({ data: { username: "user2", password: "pw", role: "USER" } }),
    prisma.user.create({ data: { username: "user3", password: "pw", role: "USER" } }),
    prisma.user.create({ data: { username: "user4", password: "pw", role: "USER" } }),
    prisma.user.create({ data: { username: "user5", password: "pw", role: "USER" } }),
  ]);
