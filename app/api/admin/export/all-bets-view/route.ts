...
export async function GET(req: NextRequest) {
  try {
    ...
    const betsForView = await prisma.bet.findMany({
      include: {
        user: {
          select: {
            username: true,
    ...
    for (const [eventId, eventBets] of groupedByEvent.entries()) {
      ...
      for (const bet of eventBets) {
        ...
        const wonCredit = isEligibleWinner
          ? Math.floor(totalDistributed / winnersCount)
          : event.finalHomeGoals !== null
            ? 0
            : "-";
    ...
    XLSX.utils.book_append_sheet(workbook, sheet, "OsszesTippekNezet");

    const fileBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "buffer" });
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const fileName = `sportfogadas-osszes-tippek-${timestamp}.xlsx`;
    ...
    console.error("Admin all-bets export error:", err);
